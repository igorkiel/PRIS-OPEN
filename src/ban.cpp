...
bool IOBan::isAccountBanned(uint32_t accountId, BanInfo& banInfo, std::string name)
{
	...
	DBResult_ptr check_name_ban = db.storeQuery(fmt::format("SELECT 1 FROM `players_ban` WHERE `name` = {:s}", db.escapeString(name)));
	
	if (!check_name_ban) {
		std::cout<<"Retornou false do check name"<<std::endl;
		return false;
	}
	...
	if (expiresAt != 0 && time(nullptr) > expiresAt) {
		...
		//g_databaseTasks.addTask(fmt::format("DELETE FROM `players_ban` WHERE `name` = {:d}", name));
		return false;
	}
