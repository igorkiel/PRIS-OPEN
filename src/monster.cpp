...
void Monster::addItem(uint16_t itemId, uint32_t count) {
    Item* newItem = Item::CreateItem(itemId, count);
    if (!newItem) {
        return;
    }

    lootItems.emplace_back(newItem);
}

void Monster::setLootMultiplier(double multiplier) {
    lootMultiplier = multiplier;
}

double Monster::getLootMultiplier() const {
    return lootMultiplier;
}


int32_t Monster::getExperience() const {
    return mType->info.experience;
}

void Monster::setExperience(int32_t experience) {
    if (originalExperience == 0) {
        originalExperience = mType->info.experience;
    }
    mType->info.experience = experience;
}
...
void Monster::setElite(bool isElite) {
    isEliteMonster = isElite;
}

int32_t Monster::getMinDamage() const {
    if (!mType->info.attackSpells.empty()) {
        return mType->info.attackSpells.front().minCombatValue;
    }
    return 0;
}

int32_t Monster::getMaxDamage() const {
    if (!mType->info.attackSpells.empty()) {
        return mType->info.attackSpells.front().maxCombatValue;
    }
    return 0;
}

void Monster::setDamage(int32_t minDamage, int32_t maxDamage) {
    for (spellBlock_t& spell : mType->info.attackSpells) {
        if (spell.originalMinCombatValue == 0 && spell.originalMaxCombatValue == 0) {
            spell.originalMinCombatValue = spell.minCombatValue;
            spell.originalMaxCombatValue = spell.maxCombatValue;
        }
        spell.minCombatValue = minDamage;
        spell.maxCombatValue = maxDamage;
    }
}

void Monster::resetDamageToOriginal() {
    if (!mType->info.attackSpells.empty()) {
        for (spellBlock_t& spell : mType->info.attackSpells) {
            spell.minCombatValue = spell.originalMinCombatValue;
            spell.maxCombatValue = spell.originalMaxCombatValue;
        }
    }
}

void Monster::resetExperienceToOriginal() {
    if (originalExperience > 0 && mType->info.experience != originalExperience) {
        mType->info.experience = originalExperience;
    }
}

int32_t Monster::getSpeed() const {
    return baseSpeed;
}

void Monster::setSpeed(int32_t speed) {
    // Armazena a velocidade original se ainda não foi definida
    if (originalSpeed == 0) {
        originalSpeed = baseSpeed;
    }

    // Atualiza a baseSpeed com a nova velocidade
    baseSpeed = speed;

    // Envia a nova velocidade ao cliente
    g_game.changeSpeed(this, getStepSpeed());
}




void Monster::resetSpeedToOriginal() {
    if (originalSpeed > 0 && static_cast<int>(baseSpeed) != originalSpeed) {
...
bool Monster::isInSpawnRange(const Position&) const
{
    return true;
}
...
void Monster::dropLoot(Container* corpse, Creature* mostDamageCreature)
{
    if (corpse && lootDrop) {
        ...
        double lootMultiplier = getLootMultiplier();
        ...
        for (Item* item : items) {
            int32_t itemCount = item->getItemCount();

            if (item->isStackable()) {
                uint16_t newCount = static_cast<uint16_t>(itemCount * lootMultiplier);
                item->setItemCount(newCount > 0 ? newCount : 1);
            }
        }
		...
		for (const auto& it : damageMap) {
			...
			if (attacker && attacker->getPlayer()) {
				attackers.insert(it.first);
			}
		}
		bool sameParty = true;
		Party* firstParty = nullptr;

		for (uint32_t attackerId : attackers) {
			Player* attackerPlayer = g_game.getPlayerByID(attackerId);
			if (attackerPlayer) {
				Party* attackerParty = attackerPlayer->getParty();
				if (attackerParty != 0) {
					if (!firstParty) {
						firstParty = attackerParty;
					} else if (firstParty != attackerParty) {
						sameParty = false;
						break;
					}
				}
		...
		if (attackers.size() > 1) {
			if (sameParty) {
				firstParty->getLeader()->doAutoLoot(items);
			}
		}
		else{
