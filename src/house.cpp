...
// Adicionada a função para acessar o número de tiles
size_t House::getHouseTileCount() const {
...
void House::setOwner(uint32_t guid, bool updateDatabase/* = true*/, Player* player/* = nullptr*/)
{
    ...
    if (updateDatabase && owner != guid) {
        ...
        db.executeQuery(fmt::format("UPDATE `houses` SET `owner` = {:d}, `guildName` = '{:s}', `bid` = 0, `bid_end` = 0, `last_bid` = 0, `highest_bidder` = 0 WHERE `id` = {:d}", 
                        guid, db.escapeString(guildName), id));
    }
            ...
            if (guildHall && player) {
                ...
                if (guild) {
                    ...
                    for (const auto& member : members) {
                        ...
                        guestList += member.name;
                    }
                    setAccessList(GUEST_LIST, guestList);
                }
            }
        }
    }
...
void House::updateDoorDescription() const
{
    ...
    const int32_t houseRent = getHouseTileCount() * g_config.getNumber(ConfigManager::HOUSE_RENT); // Aluguel com base no tamanho da casa
    std::string ownerDisplayName = "Nobody";
    ...
    for (const auto& it : doorSet) {
        std::string description = fmt::format("It belongs to house '{:s}'. {:s} owns this house.", houseName, ownerDisplayName);

        if (g_config.getBoolean(ConfigManager::HOUSE_DOOR_SHOW_PRICE) && (housePrice != -1) && (owner == 0)) {
            description += fmt::format(" It costs {:d} gold coins.", (getHouseTileCount() * housePrice));
        }

        if (houseRent > 0) {
            description += fmt::format(" The rent is {:d} gold coins.", houseRent);
        } else {
            description += " The rent is 0 gold coins.";
        }
...
void Houses::payHouses(RentPeriod_t rentPeriod) const
{
    ...
    for (const auto& it : houseMap) {
        ...
        const uint32_t rent = g_config.getNumber(ConfigManager::HOUSE_RENT) * house->getHouseTileCount(); // Use getHouseTileCount
        if (rent == 0) {
            continue;
        }
        ...
        if (difftime(paidUntil, currentTime) <= 3 * 24 * 60 * 60 && house->getPayRentWarnings() == 0) {
            Player player(nullptr);
            if (IOLoginData::loadPlayerById(&player, house->getOwner())) {
                ...
                letter->setText(fmt::format("Reminder! \nThe {:s} rent of {:d} gold for your house \"{:s}\" is due in 3 days. Please ensure you have sufficient funds to avoid losing the house.",
                                            period, rent, house->getName()));

                g_game.internalAddItem(player.getInbox(), letter, INDEX_WHEREEVER, FLAG_NOLIMIT);
                
                house->setPayRentWarnings(1);
        ...
        if (player.getBankBalance() >= rent) {
            ...
            time_t newPaidUntil = currentTime;
            switch (rentPeriod) {
                case RENTPERIOD_DAILY:
                    newPaidUntil += 24 * 60 * 60;
                    break;
                case RENTPERIOD_WEEKLY:
                    newPaidUntil += 24 * 60 * 60 * 7;
                    break;
                case RENTPERIOD_MONTHLY:
                    newPaidUntil += 24 * 60 * 60 * 30;
                    break;
                case RENTPERIOD_YEARLY:
                    newPaidUntil += 24 * 60 * 60 * 365;
                    break;
                default:
                    break;
            }
            house->setPaidUntil(newPaidUntil);
        } else {
            if (house->getPayRentWarnings() < 7) {
                ...
                letter->setText(fmt::format("Warning! \nThe {:s} rent of {:d} gold for your house \"{:s}\" is payable. Have it within {:d} days or you will lose this house.",
                                            period, rent, house->getName(), daysLeft));
