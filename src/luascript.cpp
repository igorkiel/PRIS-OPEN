...
void LuaScriptInterface::registerFunctions()
{
	...
	registerEnumIn("configKeys", ConfigManager::HOUSE_RENT)
    ...
    registerMethod("Creature", "getDummy", LuaScriptInterface::luaCreatureGetDummy);
    registerMethod("Creature", "setDummy", LuaScriptInterface::luaCreatureSetDummy);
    registerMethod("Creature", "setDummyOwner", LuaScriptInterface::luaCreatureSetDummyOwner);
    registerMethod("Creature", "removeDummy", LuaScriptInterface::luaCreatureRemoveDummy);
	...
	registerMethod("Creature", "addShader", LuaScriptInterface::luaCreatureAddShader);
	registerMethod("Creature", "hasShader", LuaScriptInterface::luaCreatureHasShader);
	...
	registerMethod("Player", "ApplySpeed", LuaScriptInterface::luaPlayerApplySpeed);
	...
	registerMethod("Monster", "getSpeed", LuaScriptInterface::luaMonsterGetSpeed);
	registerMethod("Monster", "setSpeed", LuaScriptInterface::luaMonsterSetSpeed);
	registerMethod("Monster", "getMinDamage", LuaScriptInterface::luaMonsterGetMinDamage);
	registerMethod("Monster", "getMaxDamage", LuaScriptInterface::luaMonsterGetMaxDamage);
	registerMethod("Monster", "setDamage", LuaScriptInterface::luaMonsterSetDamage);
	registerMethod("Monster", "resetDamageToOriginal", LuaScriptInterface::luaMonsterResetDamage);
	registerMethod("Monster", "setElite", LuaScriptInterface::luaMonsterSetElite);
	registerMethod("Monster", "isElite", LuaScriptInterface::luaMonsterIsElite);
	registerMethod("Monster", "setLootMultiplier", LuaScriptInterface::luaMonsterSetLootMultiplier);
	registerMethod("Monster", "getLootMultiplier", LuaScriptInterface::luaMonsterGetLootMultiplier);
	registerMethod("Monster", "addItem", LuaScriptInterface::luaMonsterAddItem);
...
{
	// item:loadAttributes(attributes)
	Item* item = getUserdata<Item>(L, 1);
	if (item) {
		size_t attrLen;
		const char* attributes = lua_tolstring(L, 2, &attrLen);

		if (attributes && attrLen > 0) {
			PropStream propStream;
			propStream.init(attributes, attrLen);

			if (!item->unserializeAttr(propStream)) {
				std::cout << "[LuaScriptInterface::luaItemLoadAttributes] WARNING: Failed to unserialize item." << std::endl;
			}
			else {
				pushBoolean(L, true);
				return 1;
...
int LuaScriptInterface::luaItemGetAttributes(lua_State* L)
{
	...
	if (item) {
		...
		item->serializeAttr(propStream);

		size_t attributesSize;
		const char* attributes = propStream.getStream(attributesSize);
...
int LuaScriptInterface::luaCreatureGetDummy(lua_State* L)
{
    Creature* creature = getUserdata<Creature>(L, 1);
    if (!creature) {
        lua_pushnil(L);
    ...
    Creature* dummy = creature->getDummy();
    ...
    pushUserdata<Creature>(L, dummy);
    setCreatureMetatable(L, -1, dummy);
    return 1;
}

int LuaScriptInterface::luaCreatureSetDummy(lua_State* L)
{
    Creature* creature = getUserdata<Creature>(L, 1);
    if (!creature) {
        lua_pushnil(L);
    ...
    Creature* dummy = getCreature(L, 2);

    if (!dummy || creature == dummy) {
        lua_pushnil(L);
        return 1;
    }

    creature->setDummy(dummy);
    dummy->setDummyOwner(creature);
...
int LuaScriptInterface::luaCreatureRemoveDummy(lua_State* L)
{
    Creature* creature = getUserdata<Creature>(L, 1);
    if (!creature) {
        lua_pushnil(L);
    ...
    if (dummy) {
        dummy->setDummyOwner(nullptr);
        creature->removeDummy();

        if (!dummy->isRemoved()) {
            g_game.removeCreature(dummy, false);
        }
    }
...
int LuaScriptInterface::luaCreatureSetDummyOwner(lua_State* L)
{
    Creature* creature = getUserdata<Creature>(L, 1);
    if (!creature) {
        lua_pushnil(L);
    ...
    Creature* owner = getCreature(L, 2);
    if (!owner) {
        lua_pushnil(L);
        return 1;
    }

    pushBoolean(L, creature->setDummyOwner(owner));
    return 1;
}
...
int LuaScriptInterface::luaMonsterGetExperience(lua_State* L) {
    ...
    int32_t experience = monster->getExperience();
...
int LuaScriptInterface::luaMonsterSetExperience(lua_State* L) {
    ...
    int32_t experience = luaL_checknumber(L, 2);
...
int LuaScriptInterface::luaMonsterGetMaxDamage(lua_State* L) {
    Monster* monster = getUserdata<Monster>(L, 1);
    if (!monster) {
        lua_pushnil(L);
        return 1;
    ...
    lua_pushinteger(L, monster->getMaxDamage());
    return 1;
}


int LuaScriptInterface::luaMonsterGetMinDamage(lua_State* L) {
    Monster* monster = getUserdata<Monster>(L, 1);
    if (!monster) {
        lua_pushnil(L);
        return 1;
    ...
    lua_pushinteger(L, monster->getMinDamage());
    return 1;
}

int LuaScriptInterface::luaMonsterGetLootMultiplier(lua_State* L)
{
    Monster* monster = getUserdata<Monster>(L, 1);
    if (!monster) {
        lua_pushnil(L);
    ...
    double multiplier = monster->getLootMultiplier();
...
int LuaScriptInterface::luaMonsterSetLootMultiplier(lua_State* L)
{
    Monster* monster = getUserdata<Monster>(L, 1);
    if (!monster) {
    ...
    double multiplier = lua_tonumber(L, 2);

    monster->setLootMultiplier(multiplier);

    lua_pushboolean(L, true);
    return 1;
}

int LuaScriptInterface::luaMonsterSetDamage(lua_State* L) {
    Monster* monster = getUserdata<Monster>(L, 1);
    if (!monster) {
        lua_pushboolean(L, false);
        return 1;
    ...
    int32_t minDamage = luaL_checkinteger(L, 2);
    int32_t maxDamage = luaL_checkinteger(L, 3);

    monster->setDamage(minDamage, maxDamage);
    lua_pushboolean(L, true);
    return 1;
}

int LuaScriptInterface::luaMonsterIsElite(lua_State* L)
{
    Monster* monster = getUserdata<Monster>(L, 1);
    if (!monster) {
        lua_pushboolean(L, false);
    ...
    bool isElite = monster->isElite();

    lua_pushboolean(L, isElite);
    return 1;
}

int LuaScriptInterface::luaMonsterSetElite(lua_State* L)
{
    Monster* monster = getUserdata<Monster>(L, 1);
    if (!monster) {
        lua_pushboolean(L, false);
    ...
    bool isElite = lua_toboolean(L, 2);
...
int LuaScriptInterface::luaMonsterResetDamage(lua_State* L) {
    Monster* monster = getUserdata<Monster>(L, 1);
    if (!monster) {
        lua_pushboolean(L, false);
        return 1;
...
int32_t LuaScriptInterface::luaMonsterGetSpeed(lua_State* L)
{
    Creature* creature = getCreature(L, 1);
    if (!creature) {
        lua_pushnil(L);
    ...
    int32_t speed = monster->getSpeed();
...
int32_t LuaScriptInterface::luaMonsterSetSpeed(lua_State* L)
{
    Creature* creature = getCreature(L, 1);
    if (!creature) {
        lua_pushnil(L);
...
int LuaScriptInterface::luaMonsterAddItem(lua_State* L) {
    Monster* monster = getUserdata<Monster>(L, 1);
    if (!monster) {
        lua_pushnil(L);
        return 1;
    ...
    uint16_t itemId = getNumber<uint16_t>(L, 2);
    uint32_t count = lua_gettop(L) >= 3 ? getNumber<uint32_t>(L, 3) : 1;

    monster->addItem(itemId, count);
    lua_pushboolean(L, true);
    return 1;
}
...
int LuaScriptInterface::luaPlayerApplySpeed(lua_State* L)
{
    Player* player = getUserdata<Player>(L, 1);
    if (!player) {
		reportErrorFunc(L, "Player not found.");
        lua_pushnil(L);
        return 1;
    }
...
int LuaScriptInterface::luaCreatureAddShader(lua_State* L)
{
    // creature:addShader(shaderId or shaderName)
    Creature* creature = getUserdata<Creature>(L, 1);
    if (!creature) {
        lua_pushnil(L);
        return 1;
    ...
    if (isNumber(L, 2)) {
        uint8_t shaderId = getNumber<uint8_t>(L, 2);
        lua_pushboolean(L, creature->addShader(shaderId));
    } else {
        std::string shaderName = getString(L, 2);
        lua_pushboolean(L, creature->addShader(shaderName));
    }
    return 1;
}

int LuaScriptInterface::luaCreatureHasShader(lua_State* L)
{
    // creature:hasShader(shaderId or shaderName)
    const Creature* creature = getUserdata<const Creature>(L, 1);
    if (!creature) {
        lua_pushnil(L);
        return 1;
    ...
    if (isNumber(L, 2)) {
        uint8_t shaderId = getNumber<uint8_t>(L, 2);
        lua_pushboolean(L, creature->hasShader(shaderId));
    } else {
        std::string shaderName = getString(L, 2);
        lua_pushboolean(L, creature->hasShader(shaderName));
    }
    return 1;
}
...
int LuaScriptInterface::luaPlayerAddShader(lua_State* L) {
    ...
    if (isNumber(L, 2)) {
        ...
        // std::cout << "Shader ID fornecido: " << static_cast<int>(shaderId) << std::endl;
    } else {
        std::string shaderName = getString(L, 2);
        Shader* shader = g_game.shaders.getShaderByName(shaderName);
        if (!shader) {
            // std::cout << "Shader com nome '" << shaderName << "' não encontrado." << std::endl;
            lua_pushnil(L);
            return 1;
        }
        shaderId = shader->id;
        // std::cout << "Shader encontrado: " << shaderName << " com ID: " << static_cast<int>(shaderId) << std::endl;
    }

    bool result = player->addShader(shaderId);
...
int LuaScriptInterface::luaVocationGetSoulGainTicks(lua_State* L)
{
    ...
    Vocation* vocation = getUserdata<Vocation>(L, 1);
    if (vocation) {
        std::cout << "Soul Gain Ticks: " << vocation->getSoulGainTicks() << std::endl;  // Adicione este log
