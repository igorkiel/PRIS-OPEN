...
{
    // Substitui o modificador de armadura pelo novo valor
    ...
    // Atualiza as estatísticas do jogador chamando a função já existente
    sendStats();
}
...
int32_t Player::getAttack() const
{
    ...
    static const slots_t attackSlots[] = {CONST_SLOT_RIGHT, CONST_SLOT_LEFT};  // Dependendo de onde as armas são equipadas
...
{
    // Substitui o modificador de ataque pelo novo valor
...
{
    // Substitui o modificador de defesa pelo novo valor
...
{
    // Acumula o modificador de attack speed
    attackSpeedModifier += modifier;

    // Garante que o modificador acumulado não seja negativo
    if (attackSpeedModifier < 0) {
        attackSpeedModifier = 0;
    }

    // Atualiza a velocidade de ataque chamando a função de atualização de status
    sendStats();  // Enviar os dados atualizados para o cliente
}
...
uint32_t Player::getAttackSpeed() const
{
    ...
    // Modificador baseado na destreza do jogador
    ret = ret - static_cast<int32_t>(ret * getCharacterStat(CHARSTAT_DEXTERITY) / 200.0);
...
void Player::addSkillAdvance(skills_t skill, uint64_t count)
{
	...
	const Item* weapon;
	const Item* ammo = 0;
	int exercise_skill = static_cast<int>(skill);
	getShieldAndWeapon(shield, weapon);

	if (weapon && weapon->getWeaponType() == WEAPON_DISTANCE) {
		ammo = inventory[CONST_SLOT_AMMO];
	}

	if (exercise_skill != 5 && weapon && weapon->getID() == 27737) {
		if (getVocation()->getId() == 1 || getVocation()->getId() == 2 || getVocation()->getId() == 5 || getVocation()->getId() == 6){
			int32_t storageValue;
			getStorageValue(29372, storageValue);
			if(storageValue < 2) {
				addStorageValue(29372, storageValue + 1);
			}
			else{
				uint32_t manaAmount = 1;
				addStorageValue(29372, 0);
				addManaSpent(manaAmount);
			return;
			}
			...
			sendTextMessage(MESSAGE_STATUS_SMALL, "you need the necessary vocation to increase magic level using this weapon.");
			return;

		}
	}	

	if (exercise_skill && count != 0) {
		if (exercise_skill == 5) {
			if (shield && shield->getID() == 27758) {
				count *= 2;
		...
		else if (exercise_skill >= 0 && exercise_skill <= 4) {
			if ((weapon && weapon->getID() >= 27732 && weapon->getID() <= 27735)) {
				count *= 2;
				g_events->eventPlayerOnGainSkillTries(this, skill, count);
			}
			else {
...
void Player::onWalk(Direction& dir)
{
    ...
    // Somente define um próximo tempo de ação se for uma ação de andar, não para troca de itens
    if (!isPlayerWalking) {
...
void Player::addExperience(Creature* source, uint64_t exp, bool sendText/* = false*/)
{
    ...
    // Adiciona print para depuracao de soul
                             ...
                             bool checkDefense /* = false*/, bool checkArmor /* = false*/, bool field /* = false*/, bool ignoreResistances /* = false*/)
{
	...
	if (combatType == COMBAT_FIREDAMAGE){
		int fire_storage = 8372;
		player->getStorageValue(fire_storage, storageValue); // storageValue
		if (storageValue > 0){
			damage -= std::round(damage * (storageValue / 100.));
		}
	} 
	else if(combatType == COMBAT_DEATHDAMAGE){
		int death_storage = 8373;
		player->getStorageValue(death_storage, storageValue); // storageValue
		if (storageValue > 0){
			damage -= std::round(damage * (storageValue / 100.));
		}
	}
	else if(combatType == COMBAT_EARTHDAMAGE){
		int earth_storage = 8374;
		player->getStorageValue(earth_storage, storageValue); // storageValue
		if (storageValue > 0){
			damage -= std::round(damage * (storageValue / 100.));
		}
	}
	else if(combatType == COMBAT_ENERGYDAMAGE){
		int energy_storage = 8375;
		player->getStorageValue(energy_storage, storageValue); // storageValue
		if (storageValue > 0){
			damage -= std::round(damage * (storageValue / 100.));
		}
	}
	else if(combatType == COMBAT_ICEDAMAGE){
		int ice_storage = 8376;
		player->getStorageValue(ice_storage, storageValue); // storageValue
		if (storageValue > 0){
			damage -= std::round(damage * (storageValue / 100.));
		}
	}
	else if(combatType == COMBAT_HOLYDAMAGE){
		int holy_storage = 8377;
		player->getStorageValue(holy_storage, storageValue); // storageValue
		if (storageValue > 0){
			damage -= std::round(damage * (storageValue / 100.));
		}
	}
	else if(combatType == COMBAT_PHYSICALDAMAGE){
		int physical_storage = 8378;
		player->getStorageValue(physical_storage, storageValue); // storageValue
		if (storageValue > 0){
			damage -= std::round(damage * (storageValue / 100.));
		}
	}
...
ReturnValue Player::queryAdd(int32_t index, const Thing& thing, uint32_t count, uint32_t flags, Creature*) const
{
    ...
    // Verificações de bloqueio para slot de mãos (right/left)
    if ((slotPosition & SLOTP_LEGS) || (slotPosition & SLOTP_ARMOR) || (slotPosition & SLOTP_HEAD) || (slotPosition & SLOTP_FEET)) {
        // Se o item for legs, armor, ou head, bloqueia nos slots de mãos (right/left)
    ...
    // Verificações gerais de onde os itens podem ser equipados
...
uint64_t Player::calculateBonusExperience(uint64_t baseExp, bool isNightTime) const {
    int bonusId = isNightTime ? 8379 : 8380; // 8379 = bonus noturno, 8380 = bonus diurno

    int storageValue = 0;
    if (!getStorageValue(bonusId, storageValue) || storageValue <= 0) {
        return 0;
    }
    double bonusPercentage = static_cast<double>(storageValue) / 100.0;
    return static_cast<uint64_t>(std::round(baseExp * bonusPercentage));
}
...
void Player::onGainExperience(uint64_t gainExp, Creature* target)
{
    ...
    gainExp += calculateBonusExperience(gainExp, isNightTime);
