...
bool Game::removeCreature(Creature* creature, bool isLogout /* = true */)
{
    ...
    // Trigger evento de remoção para todos os espectadores
    for (Creature* spectator : spectators) {
        spectator->onRemoveCreature(creature, isLogout);
    }

    // Remove a referência do master (se aplicável)
    ...
    creature->removeList();
    creature->setRemoved();
    ReleaseCreature(creature);  // Libera a memória da criatura

    removeCreatureCheck(creature);  // Realiza verificações finais

    // Remove qualquer invocação (summons) de forma recursiva
...
void Game::playerToggleOutfitExtension(uint32_t playerId, int mount, int wings, int aura, int shader)
{
    ...
    // Alternar montaria
    if (mount != -1) {
        player->toggleMount(mount == 1);  // Monta ou desmonta
    }

    // Alternar wings
    if (wings != -1) {
        player->toggleWing(wings == 1);  // Equipa ou desequipa wings
    }

    // Se você tiver uma lógica para auras, também pode alternar aqui
    if (aura != -1) {
        // player->toggleAura(aura == 1);  // Equipa ou desequipa auras (se necessário)
    }

    // Se você tiver shaders, adicione a lógica para eles também
    if (shader != -1) {
        // player->toggleShader(shader == 1);  // Equipa ou desequipa shaders (se necessário)
    }
}
void Game::playerChangeOutfit(uint32_t playerId, Outfit_t outfit)
{
  ...
  // Wings (nova lógica)
    if (outfit.lookWings != 0) {
        ...
        if (currentWingId != 0) {
            Wing* prevWing = wings.getWingByID(currentWingId);
            if (prevWing) {
                changeSpeed(player, wing->speed - prevWing->speed);  // Ajusta a velocidade
            }
        } else {
            changeSpeed(player, wing->speed);  // Aplica a velocidade da wing
        }

        player->setCurrentWing(wing->id);
    } else if (player->getCurrentWing() != 0) {
        // Remover wings, restaurar a velocidade
        Wing* currentWing = wings.getWingByID(player->getCurrentWing());
        if (currentWing) {
            changeSpeed(player, -currentWing->speed);  // Remove a velocidade da wing
        }
	...
	if (outfit.lookAura != 0) {
		...
		if (currentAuraId != 0) {
			Aura* prevAura = auras.getAuraByID(currentAuraId);
			if (prevAura) {
				changeSpeed(player, aura->speed - prevAura->speed);  // Ajusta a velocidade
			}
		} else {
			changeSpeed(player, aura->speed);  // Aplica a velocidade da aura
		}

		player->setCurrentAura(aura->id);
	} else if (player->getCurrentAura() != 0) {
		// Remover aura, restaurar a velocidade
		Aura* currentAura = auras.getAuraByID(player->getCurrentAura());
		if (currentAura) {
			changeSpeed(player, -currentAura->speed);  // Remove a velocidade da aura
		}
		player->setCurrentAura(0);
	}

	// Shader (sem mudança de velocidade)
	...
	// Atualiza o outfit padrão do jogador
...
void Game::changeSpeed(Creature* creature, int32_t varSpeedDelta)
{
    ...
    // Log para depuração
    // std::cout << "Mudando Velocidade - Velocidade Variável Atual: " << varSpeed << " | Delta: " << varSpeedDelta << std::endl;
    ...
    if (damage.primary.value > 0) {
        ...
        Monster* monster = attacker ? attacker->getMonster() : nullptr;
         if (monster && monster->isElite()) {
            ...
            int32_t minDamage = monster->getMinDamage();
            int32_t maxDamage = monster->getMaxDamage();

            int32_t monsterDamage = uniform_random(minDamage, maxDamage);

            if (monster->getLevel() > 0) {
                float bonusDmg = g_config.getFloat(ConfigManager::MLVL_BONUSDMG) * monster->getLevel();
                if (bonusDmg != 0.0) {
                    monsterDamage += std::round(monsterDamage * bonusDmg);
                }
            }

            damage.primary.value = std::abs(monsterDamage);
        }
...
void Game::updatePlayerHelpers(const Player& player)
{
    ...
    if (Party* party = player.getParty()) {
        ...
        for (Player* member : members) {
            if (member != &player) {
                member->sendCreatureHelpers(creatureId, helpers);
            }
        }

        if (Player* leader = party->getLeader()) {
            if (leader != &player) {
                leader->sendCreatureHelpers(creatureId, helpers);
            }
        }
    }
}
...
void Game::updateCreatureType(Creature* creature)
{
    ...
    const Player* dummyOwnerPlayer = nullptr;

    uint32_t creatureId = creature->getID();
    CreatureType_t creatureType = creature->getType();

    // Verifica se a criatura é um monstro e tem um master (criatura invocada)
    ...
    const Creature* dummy = creature->getDummy();
    if (dummy) {
        const Creature* dummyOwner = dummy->getDummyOwner();
        if (dummyOwner) {
            dummyOwnerPlayer = dummyOwner->getPlayer();
            creatureType = CREATURETYPE_SUMMON_OTHERS;
        }
    }
    ...
    if (creatureType == CREATURETYPE_SUMMON_OTHERS) {
        for (Creature* spectator : spectators) {
            ...
            if (dummyOwnerPlayer && dummyOwnerPlayer == player) {
                player->sendCreatureType(dummy->getID(), CREATURETYPE_SUMMON_OWN);
            }
            else if (masterPlayer && masterPlayer == player) {
                player->sendCreatureType(creatureId, CREATURETYPE_SUMMON_OWN);
            }
            else {
                player->sendCreatureType(dummy ? dummy->getID() : creatureId, CREATURETYPE_SUMMON_OTHERS);
            }
        }
    } else {
        for (Creature* spectator : spectators) {
