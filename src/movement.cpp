...
{
    // Verificar se o item tem atributos que justificam a criação de um evento de equipamento/desquipamento
    if (itemType.weaponType != WEAPON_NONE || itemType.ammoType != AMMO_NONE || itemType.attack != 0 ||
        ...
        // Lista de eventos de equipamento/desquipamento a serem registrados
        static const std::vector<MoveEvent_t> eventList = {MOVE_EVENT_EQUIP, MOVE_EVENT_DEEQUIP};

        for (auto& itEventId : eventList) {
            // Criar um novo evento de MoveEvent para o item
            ...
            if (event) {
                ...
                // Definir a função de equipamento ou desquipamento
                ...
                // Definir o slot correto baseado na posição do item
                if (itemType.slotPosition & SLOTP_NECKLACE) {
                    ...
                    return; // Se não for um item de equipamento válido, sair
                }

                // Adicionar as informações de wieldInfo
                ItemType& it = Item::items.getItemType(itemType.id);
                it.wieldInfo |= moveEvent->getWieldInfo();
                it.minReqLevel = std::max(it.minReqLevel, moveEvent->getReqLevel());
                it.minReqMagicLevel = std::max(it.minReqMagicLevel, moveEvent->getReqMagLv());

                // Adicionar as vocações diretamente, sem adicionar uma nova linha
                if (!moveEvent->getVocationString().empty()) {
                    it.vocationString += moveEvent->getVocationString(); // Concatenar vocações diretamente
                }

                // Adicionar o evento ao mapa de eventos
	...
	if ((attr = node.attribute("itemid"))) {
		...
		for (const auto& id : idList) {
			if (moveEvent->getEventType() == MOVE_EVENT_EQUIP) {
				...
				// Atualizar wieldInfo com informações de nível e magic level
				it.wieldInfo = moveEvent->getWieldInfo();
				it.minReqLevel = moveEvent->getReqLevel();
				it.minReqMagicLevel = moveEvent->getReqMagLv();

				// Adicionando vocações do XML ao vocEquipMap e construindo a string de vocações
				std::string vocations;
				for (auto vocationNode : node.children("vocation")) {
					std::string vocName = vocationNode.attribute("name").as_string();
					moveEvent->addVocEquipMap(vocName);
				...
				// Atualizar a vocação corretamente
				if (!vocations.empty()) {
					it.vocationString = vocations; // Sobrescreve a string existente para evitar duplicações
				}
			}
		...
		if (moveEvent->getEventType() == MOVE_EVENT_EQUIP) {
			...
			if (!vocations.empty()) {
				it.vocationString = vocations; // Sobrescreve a string existente
			}
			...
			while (++id <= endId) {
				...
				tit.vocationString = it.vocationString;  // Replicar vocações para o range
			}
		} else {
			while (++id <= endId) {
				addEvent(*moveEvent, id, itemIdMap);
...
//		}
    ...
    int32_t speedValue = it.abilities->speed + item->getAttributeValue(TOOLTIP_ATTRIBUTE_SPEED);
    if (speedValue != 0) {
        g_game.changeSpeed(player, speedValue);
    }

    uint32_t attackSpeedModifier = item->getAttributeValue(TOOLTIP_ATTRIBUTE_ATTACK_SPEED);
    if (attackSpeedModifier != 0) {
        player->setAttackSpeedModifier(static_cast<int32_t>(attackSpeedModifier));
        needUpdateStats = true;
    }

    int32_t armorModifier = item->getAttributeValue(TOOLTIP_ATTRIBUTE_ARMOR);
    if (armorModifier != 0) {
        player->setArmorModifier(armorModifier);
        needUpdateStats = true;
    }

    int32_t attackModifier = it.abilities->attack + item->getAttributeValue(TOOLTIP_ATTRIBUTE_ATTACK);
    if (attackModifier != 0) {
        player->setAttackModifier(attackModifier);
        needUpdateStats = true;
    }

    int32_t defenseModifier = it.abilities->defense + item->getAttributeValue(TOOLTIP_ATTRIBUTE_DEFENSE);
    if (defenseModifier != 0) {
        player->setDefenseModifier(defenseModifier);
        needUpdateStats = true;
    }
