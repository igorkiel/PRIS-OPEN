...
void Container::onRemoveContainerItem(uint32_t index, Item* item) {
    ...
    for (Creature* spectator : spectators) {
        ...
        spectatorPlayer->sendRemoveContainerItem(this, index);
		spectatorPlayer->onRemoveContainerItem(this, item);
    }

    Creature* creature = dynamic_cast<Creature*>(getTopParent());
	if (!creature){
		return;
	}
    ...
    int containerID = this->getID();
    if (containerID && containerID == 26052) {
        StoreInbox::removeEffects(item, player);
    }
}
		...
		uint32_t flags, Creature* actor/* = nullptr*/) const
{
    ...
    // Check if the container is a GOLD_POUCH and the item is not allowed
    if (getID() == 28165) { // Substitua 28165 pelo ID da sua GOLD_POUCH
        if (item->getID() != 2148 && item->getID() != 2152 && item->getID() != 2160) {
            return RETURNVALUE_CANNOTMOVETOGOLDPOUCH;
        }
    }
