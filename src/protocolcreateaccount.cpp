...
void ProtocolCreateAccount::doCreateAccount(const std::string& accountName, const std::string& email, const std::string& password, const std::string& passwordConfirmation, uint16_t version)
{
    ...
    // Gerar a chave de recuperação
    std::string recoveryKey = IOCreateAccountData::generateRecoveryKey();

    if (!IOCreateAccountData::insertAccount(accountName, email, password, recoveryKey)) {
        disconnectClient("Something went wrong, please contact administrator!", version);
        return;
    }

    // Enviar a chave de recuperação por e-mail
    if (!IOCreateAccountData::sendRecoveryKeyEmail(email, recoveryKey)) {
        disconnectClient("Failed to send recovery key email.", version);
        return;
    }

    auto output = OutputMessagePool::getOutputMessage();
    output->addByte(0xFF);
    output->addString("Your account has been created.");
    send(output);
    disconnect();
}
