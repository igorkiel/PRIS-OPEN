...
bool Combat::isProtected(const Player* attacker, const Player* target)
{
    ...
    // Remove ou comente as seguintes linhas para permitir ataques entre todas as vocações e jogadores sem vocação
    // /*
...
ReturnValue Combat::canTargetCreature(Player* attacker, Creature* target)
{
    ...
    if (target->isDummy()) {
        Creature* dummyOwner = target->getDummyOwner();
        if (dummyOwner == attacker) {
    ...
    if (attacker->isDummy()) {
        Creature* dummyOwner = attacker->getDummyOwner();
        if (dummyOwner == target) {
            return Combat::canDoCombat(attacker, target);
        }
        return RETURNVALUE_YOUMAYNOTATTACKTHISPLAYER;
    }
...
ReturnValue Combat::canDoCombat(Creature* attacker, Creature* target) {
    ...
    // Verificações adicionais de combate
