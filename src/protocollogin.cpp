...
void ProtocolLogin::onRecvFirstMessage(NetworkMessage& msg)
{
	...
	if (var == 1) {
		...
		DailyMonsterList list = g_game.getDailyMonsterList();
		output->addByte(list.size());
		for (auto& [name, multipliers] : list)
		{
			const auto& mType = g_monsters.getMonsterType(name);
			output->addString(name);
			output->add<uint16_t>(mType->info.outfit.lookType);
			if (mType->info.outfit.lookType != 0)
			{
				output->addByte(mType->info.outfit.lookHead);
				output->addByte(mType->info.outfit.lookBody);
				output->addByte(mType->info.outfit.lookLegs);
				output->addByte(mType->info.outfit.lookFeet);
				output->addByte(mType->info.outfit.lookAddons);
				output->add<uint16_t>(mType->info.outfit.lookMount);
			}
			else if (mType->info.outfit.lookTypeEx != 0)
			{
				const ItemType& it = Item::items[mType->info.outfit.lookTypeEx];
				output->add<uint16_t>(it.clientId);
			}
			else
			{
				output->add<uint16_t>(mType->info.outfit.lookTypeEx);
			}

			for (auto& [id, multiplier] : multipliers)
			{
				output->addByte(multiplier);
			}
		}
