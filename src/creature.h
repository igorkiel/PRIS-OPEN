...
class Creature : virtual public Thing
{
	...
	public:
		...
		bool hasShader(uint8_t shaderId) const;
		...
		bool setDummy(Creature* newDummy);
		...
		bool isDummy() const {
			 return dummyOwner != nullptr;
		}

		Creature* getDummy() const {
		if (dummy) {
				} else {
			}
			return dummy;
        ...
        bool setDummyOwner(Creature* owner);
        Creature* getDummyOwner() const {
            return dummyOwner;
        }
	...
	protected:
        ...
        Creature* dummy = nullptr;
		Creature* followCreature = nullptr;
		Creature* dummyOwner = nullptr;
