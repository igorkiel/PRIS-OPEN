...
class Player final : public Creature, public Cylinder
{
	...
	std::string getGuildName() const;
	  ...
	  bool toggleWing(bool equip);  // Função para equipar/desequipar wing
      // bool toggleAura(uint8_t auraId, bool equip);  // Função para equipar/desequipar aura
		uint8_t getCurrentMount() const;
		void setCurrentMount(uint8_t mountId);
		void applySpeed();  // Declaração da função applySpeed
		...
		int32_t setMaxHealth(int bonus) {
			uint32_t vitHealth = static_cast<uint32_t>(healthMax * (charStats[CHARSTAT_VITALITY] / 200.f));
			healthMax += bonus;
			return std::max<int64_t>(1, healthMax + varStats[STAT_MAXHITPOINTS] + vitHealth);
		}
		...
		uint64_t calculateBonusExperience(uint64_t baseExp, bool isNightTime) const;
...
void sendQuiverUpdate(bool sendAll = false)
	{
	    ...
	    void setAttackSpeedModifier(int32_t modifier);
	    void setArmorModifier(int32_t modifier);
		int32_t getAttack() const;
	    void setAttackModifier(int32_t modifier);
	    void setDefenseModifier(int32_t modifier);
	...
	private:
        ...
        int32_t attackSpeedModifier = 0; 
        int32_t armorModifier = 0;
        int32_t attackModifier = 0;  // Modificador de ataque
        int32_t defenseModifier = 0; // Modificador de defesa
		uint8_t soul = 0;
