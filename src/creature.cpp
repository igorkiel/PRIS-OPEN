...
bool Creature::addShader(uint8_t shaderId)
{
    ...
    Outfit_t outfit = getCurrentOutfit();
    outfit.lookShader = shader->id;
    g_game.internalCreatureChangeOutfit(this, outfit);
    return true;
}

bool Creature::addShader(const std::string& shaderName)
{
...
bool Creature::hasShader(uint8_t shaderId) const
{
    Outfit_t outfit = getCurrentOutfit();
    return outfit.lookShader == shaderId;
}

bool Creature::hasShader(const std::string& shaderName) const
{
    Shader* shader = g_game.shaders.getShaderByName(shaderName);
    if (!shader) {
        return false;
    ...
    Outfit_t outfit = getCurrentOutfit();
    return outfit.lookShader == shader->id;
}
...
void Creature::onDeath()
{
    ...
    if (monster && monster->isElite()) {
        ...
        monster->resetExperienceToOriginal();
        monster->resetSpeedToOriginal();
    }
...
void Creature::removeDummy() {
    if (dummy) {

        // Desassociar o dummy do jogador (remove o jogador como dono do dummy)
        dummy->setDummyOwner(nullptr);  // Limpa a referência ao dono no dummy
        dummy = nullptr;  // Limpa a referência ao dummy no jogador

        // Verificação final para garantir que o dummy foi removido corretamente
        if (!hasDummy()) {
        } else {
        }
    } else {
...
bool Creature::setDummy(Creature* newDummy) {
    if (this == newDummy) {
        return false;
    }
    ...
    if (newDummy) {
        ...
        // Define o jogador como dono do dummy
        newDummy->setDummyOwner(this);
        incrementReferenceCounter();  // Incrementa o contador de referência do dummy

        // Verificação extra
        if (newDummy->getDummyOwner() != this) {
            return false;
        }
    } else {
        return false;
...
bool Creature::setDummyOwner(Creature* owner) {
    if (owner == nullptr) {
        // Limpar a referência ao dummy no jogador, se o dummy ainda estiver associado
        if (dummyOwner) {
            dummyOwner->setDummy(nullptr);  // Remove a referência ao dummy no jogador anterior
        }

        dummyOwner = nullptr;  // Limpar a referência ao dono no dummy

        // Verificação extra para garantir que o dummy esteja correto
        if (dummyOwner != nullptr) {
            return false;
        }
    ...
    dummyOwner = owner;  // Setar o novo dono
    return true;
}
...
void Creature::setIcon(int8_t new_icon)
{
	icon = new_icon;
