...
function onUseShovel(player, item, fromPosition, target, toPosition, isHotkey)
	...
	elseif targetId == 10037 and target:getActionId() == 50117 then
		local coalItem = Tile(Position(32699, 31493, 11)):getItemById(10037)
		local coalItem_pos = coalItem:getPosition()
		if coalItem then
			coalItem:remove()
			local new_coalItem = Game.createItem(7131, 1, coalItem_pos)
			if new_coalItem then
				new_coalItem:setActionId(50117)
			end
			toPosition:sendMagicEffect(CONST_ME_POFF)

			local crucibleItem = Tile(Position(32699, 31495, 11)):getItemById(8642)
			if crucibleItem then
				local cruciblePos = crucibleItem:getPosition()
				cruciblePos:sendMagicEffect(CONST_ME_POFF)
...
function onUsePick(player, item, fromPosition, target, toPosition, isHotkey)
	...
	elseif targetId == 1304 then
		...
		elseif targetActionId == 50058 then
			local stoneStorage = Game.getStorageValue(GlobalStorage.NaginataStone) or 0 -- Usar 0 como valor padrão se for nil
			...
			elseif stoneStorage == 5 then
				target:remove()
				Game.setStorageValue(GlobalStorage.NaginataStone, 0) -- Resetar o valor
			end
	...
	elseif targetActionId == 50114 then
		--if Tile(Position(32617, 31514, 9)):getItemById(1027) and Tile(Position(32617, 31514, 9)):getItemById(1205) then
			local rubbleItem = Tile(Position(32619, 31515, 9)):getItemById(5709)
			if rubbleItem then
				rubbleItem:remove()
			end
		--else
