...
local function creatureSayCallback(cid, type, msg)
   ...
   elseif npcHandler.topic[cid] == topicList.TRANSFER_PLAYER_CONSENT then
    if msgcontains(msg, "yes") then
        ...
        if not player:transferMoneyTo(transfer[cid].name, count[cid]) then
            npcHandler:say("You cannot transfer money to this account. The player may not exist or the database update failed.", cid)
        else
            npcHandler:say("Very well. You have transferred " .. count[cid] .. " gold to " .. transfer[cid].name .. ".", cid)
            transfer[cid] = nil
        end
    elseif msgcontains(msg, "no") then
