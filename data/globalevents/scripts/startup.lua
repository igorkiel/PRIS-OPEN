function onStartup()
	...
	-- Check for houses with non-premium owners
--[[	
	local resultId = db.storeQuery("SELECT `id`, `owner` FROM `houses` WHERE `owner` != 0")
	if resultId ~= false then
		repeat
			local houseId = result.getNumber(resultId, "id")
			local ownerId = result.getNumber(resultId, "owner")

			-- Check if the owner has a premium account
			local premiumResultId = db.storeQuery("SELECT `premium_ends_at` FROM `accounts` WHERE `id` = (SELECT `account_id` FROM `players` WHERE `id` = " .. ownerId .. ")")
			getPlayer
			if premiumResultId ~= false then
				local isPremium = result.getNumber(premiumResultId, "premium_ends_at")
				result.free(premiumResultId)

				-- If the owner is not premium, set the house owner to 0
				if isPremium == 0 then
					db.query("UPDATE `houses` SET `owner` = 0 WHERE `id` = " .. houseId)
				end
			end
		until not result.next(resultId)
		result.free(resultId)
