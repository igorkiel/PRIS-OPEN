...
function init()
  ...
  if not g_app.isMobile() then
    questLogButton = modules.client_topmenu.addLeftButton('questLogButton', tr('Quest Log'), '/images/topbuttons/questlog', function() toggleQuestLog() end, false, 8)
    questTrackerButton = modules.client_topmenu.addLeftButton('questTrackerButton', tr('Quest Tracker'), '/images/topbuttons/quest_tracker', toggle, false, 9)
  end
...
function showQuestLine()
  ...
  if not child then
    -- g_logger.error("Erro: Nenhum item está selecionado na lista de quests.")
    return
  end
