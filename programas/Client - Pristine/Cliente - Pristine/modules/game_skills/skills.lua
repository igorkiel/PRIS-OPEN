...
function update()
  local attackspeed = skillsWindow:recursiveGetChildById('attackspeed')

  -- Aqui, removemos a verificação da feature e garantimos que o widget vai aparecer
  attackspeed:show()
...
function refresh()
  ...
  onArmorChange(player, player:getArmor())  -- Atualiza o valor de armadura
...
function onArmorChange(localPlayer, armor)
  local armorWidget = skillsWindow:recursiveGetChildById('armor')

  if armorWidget then
    setSkillValue('armor', armor)
  end
end
