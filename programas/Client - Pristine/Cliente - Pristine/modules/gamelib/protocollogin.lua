...
LoginServerDailyMonstersRequest = 96
...
function ProtocolLogin:dailyMonster(host, port)
	self.connectCallback = self.sendDailyMonsterRequestPacket
	self:connect(host, port)
end
...
function ProtocolLogin:sendDailyMonsterRequestPacket()
	self:sendPacket(1)
end

function ProtocolLogin:sendPacket(id)
...
function ProtocolLogin:onRecv(msg)
  while not msg:eof() do
    ...
    elseif opcode == LoginServerDailyMonstersRequest then
			self:parseDailyMonsterRequest(msg)
...
function ProtocolLogin:parseDailyMonsterRequest(msg)
	local dailyMonsters = {}
	local amount = msg:getU8()
	for i = 1, amount do
		local outfit = {}
		local dailyMonster = {}

		dailyMonster.name = msg:getString()
		...
		if outfit.type ~= 0 then
			...
			outfit.mount = msg:getU16()
		else
			outfit.auxType = msg:getU16()
		end

		dailyMonster.damage = msg:getU8()
		dailyMonster.health = msg:getU8()
		dailyMonster.experience = msg:getU8()
		dailyMonster.loot = msg:getU8()
		dailyMonster.spawn = msg:getU8()
		dailyMonsters[i] = {dailyMonster, outfit}
	end

	signalcall(self.onDailyMonsterRequest, self, dailyMonsters)
end
