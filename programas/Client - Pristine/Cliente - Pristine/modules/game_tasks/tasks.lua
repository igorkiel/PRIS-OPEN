...
local tasks = {}
local activeTasks = {}
local completedTasks = {} -- Nova tabela para armazenar tarefas completas
local playerLevel = 0
local RewardType = {
  Points = 1,
  Experience = 2,
...
function destroy()
  ...
  tasks = {}
  activeTasks = {}
  completedTasks = {} -- Limpar a tabela de tarefas completas
  playerLevel = 0
  jsonData = ""
end
...
function onTasksList(data)
  ...
  for taskId, task in ipairs(data) do
    ...
    if task.completed then
      widget:setBackgroundColor("#00FF00") -- Cor verde
      widget:setEnabled(false) -- Desabilita a seleção
    end
  end
...
function onTaskSelected(parent, child, reason)
  ...
  -- Verifica se a tarefa foi concluída e desabilita o botão de start
  if task.completed then
    tasksWindow.start:setEnabled(false)
  else
    tasksWindow.start:setEnabled(true)
  end
end
